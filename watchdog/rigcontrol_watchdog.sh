...
# rigcontrol-watchdog.conf - one [algoname] block per algorithm. Overwritten
# by the dashboard's Watchdog Config module on "Apply to Selected Rigs".
# An algorithm with no block here is not monitored.
EOF
