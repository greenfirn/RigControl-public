# no-container-docker_events_monitor--no-screen-vast.sh
# No-screen variant (miner runs as a plain background process, no GNU screen
# session) with the Vast.ai idle-image filter merged in: Vast's own
# benchmark/test containers (bandwidth-test, speedtest, common) are ignored
# when deciding whether the rig is idle, so they don't block the miner from
# starting.
...
# Containers whose image should NOT count toward "something is running" -
# Vast.ai's own benchmark/test containers. Matched by EXACT image name (not
# prefix) - keep this in sync with whatever Vast actually launches.
IGNORED_IMAGES=(
...
should_ignore_image() {
    ...
    # Exact match against IGNORED_IMAGES, matching vast.sh's own filter.
...
# Check if ANY container is running, excluding IGNORED_IMAGES (e.g. Vast's
# benchmark/test containers) so those don't count as "busy" on their own.
