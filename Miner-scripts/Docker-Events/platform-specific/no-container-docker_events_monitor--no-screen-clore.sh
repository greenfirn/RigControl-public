# no-container-docker_events_monitor--no-screen-clore.sh
# No-screen variant (miner runs as a plain background process, no GNU screen
# session) with the Clore.ai idle-image filter merged in: the monitoring
# sidecar container (cloreai/monitoring) is ignored when deciding whether
# the rig is idle, so its own always-on container doesn't block the miner
# from starting.
...
# e.g. Clore.ai's own always-on monitoring sidecar. Any running container
# whose image starts with one of these prefixes is skipped when deciding
# whether the rig is idle.
IGNORED_IMAGES=(
    "cloreai/monitoring"
)
...
# Check if ANY container is running, excluding IGNORED_IMAGES (e.g. Clore's
# monitoring sidecar) so that container doesn't count as "busy" on its own.
