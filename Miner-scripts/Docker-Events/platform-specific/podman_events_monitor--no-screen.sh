# Nosana-podman_events_monitor--no-screen.sh
# No-screen variant (miner runs as a plain background process via setsid,
# no GNU screen session) of the Nosana/podman-in-docker events monitor.
# The podman idle-detection logic (confirm_podman_idle, child-container
# checks, etc.) is unchanged - only start_miner/stop_miner/kill_by_pid were
# converted, matching the same no-screen treatment used for the plain
# docker_events monitor variants.
