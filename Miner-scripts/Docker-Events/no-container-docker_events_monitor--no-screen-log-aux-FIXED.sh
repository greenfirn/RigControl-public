...
# rig-cpu.json) so CPU and GPU rigs each check their own JSON file.
...
# Miner binary version numbers (XMRIG_VERSION etc.) have no rig-gpu.json
# equivalent by design - this file must exist regardless of whether the
# rig is running JSON-only or .conf-only for everything else.
...
add_api_flags() {
    ...
    case "$miner_name" in
        ...
        "wildrig"|"wildrig-multi")
            ...
            # started with no --api-port flag at all (confirmed live:
            # journal showed "No WILDRIG_MULTI_API_PORT found ... API
            # disabled" -> API_PORT=0 -> add_api_flags() never even called).
...
# NOTE: SCREEN_NAME has no rig-gpu.json equivalent yet (not in RIG_GPU_JSON_KEYS in
# 00-get_rig_conf.sh), so on a JSON-only rig (no .conf file) this always comes back
# empty from get_rig_conf() itself.
#
# SLOT name ("gpu"/"cpu"/"aux", derived from OC_FILE) is now the primary
