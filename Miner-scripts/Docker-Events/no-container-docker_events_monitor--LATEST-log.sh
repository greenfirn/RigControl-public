...
# Always tee the miner's screen output to a log file, even for miners with a
# known API - makes it easy to review recent miner output (e.g. `tail -f`)
# without attaching to the screen session. Hardcoded on by default; the API
# is still used for stats/health checks regardless of this setting.
ALWAYS_LOGS=true
