...
# trimmed back down to the tail end. Without a screen session to attach to,
# and without a systemd journal backing this standalone script, the log file
# is the only persistent record of miner output - it needs to self-trim.
...
# No OC_FILE indirection in this standalone script - CFG_FILE is the
# hardcoded rig-conf path, so the slot is derived from it instead.
...
start_miner() {
    ...
    # No screen session in this variant, and no systemd journal either (this is a
    # standalone manual script, not a service) - so miner output is always teed to
    # $LOG_FILE, which self-trims via MAX_LOG_BYTES/LOG_CHECK_INTERVAL. Without this,
    # output would go to whatever terminal invoked this script, which may close and
    # break the pipe once this script itself exits.
    rm -f "$LOG_FILE"
