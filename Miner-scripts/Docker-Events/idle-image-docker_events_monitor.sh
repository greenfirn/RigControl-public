...
# Always tee the miner's screen output to a log file - makes it easy to
# review recent miner output without attaching to the screen session.
# Hardcoded on by default; the API, if present, is still used for stats.
ALWAYS_LOGS=true
