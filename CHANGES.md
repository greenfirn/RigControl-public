...
## Custom-miner API/BIN/LOG conf settings could still need an agent restart despite the PID-based version refresh

**What:** while testing the `keryx-miner` log-regex fix above, restarting just the miner (not the whole `rigcontrol-agent` service) wasn't enough to pick up a newly-added `KERYX_MINER_API_PORT` - an agent restart was still needed. This looked like it should've already been covered by the earlier PID-based force-refresh work (`_custom_miner_last_pid`), but that mechanism only ever force-refreshed `_named_miner_version()`'s cache; `_read_agent_conf_val()` (which every `<NAME>_API_HOST`/`_PORT`/`_BIN`/`_LOG_PATH` lookup goes through) has its own independent mtime-diff cache that isn't tied to PID changes at all - it only re-reads when the conf file's mtime itself has changed since the last read, which is usually true but isn't guaranteed to line up with the exact moment a miner restarts.

**Fixed:** `detect_running_miners()`'s PID-change branch now also resets `_agent_conf_cache["mtime"] = None` alongside the existing `_named_miner_version(..., force=True)` call, so the very next `_read_agent_conf_val()` call for that miner unconditionally re-reads the conf file from disk instead of trusting its cached mtime comparison. A miner restart (crash-restart, reinstall to a new version, or the Docker-Events launcher stopping/starting it) is exactly the moment a related conf edit is most likely to need to take effect immediately, rather than waiting on whatever the next incidental conf write happens to be.

**Verified:** functional test confirming the PID-change branch resets the cache mtime to `None` and that the following `_read_agent_conf_val()` call reflects a conf edit made between the two events, using a real `detect_running_miners()` call against a mocked `ps aux` line. `bash -n` clean on `rigcontrol_telemetry.sh`/`rigcontrol_telemetry-exclude.sh`/`copy-paste-update.sh`; `python3 -m py_compile` clean on all 3 extracted bodies; confirmed all 3 bodies are byte-identical to each other.

## Shortened accepted/rejected/stale/invalid share counts to K/M/B/T in the dashboard

**What:** raw share counts (e.g. `69059993440`) were being rendered unshortened in several places, most visibly the compact rig-list row's "CPU | GPU | AUX" column, even though the equivalent hashrate values already get unit-shortened.

**Fixed:** added `fmtShareCount(n)` to `static/js/app.js` (K/M/B/T threshold-based, `>= 1e6` before shortening, `.toFixed(2)` + suffix) and applied it everywhere a share count renders as a visible number: `fmtShares()`, `DataHelper.getFormattedShares()`, the miner-row-horizontal SHARES tile, and (found in a second pass, after the first round missed it) `DataHelper.getCpuColumnContent`/`getGpuColumnContent`/`getAuxColumnContent`'s compact-row HTML. Tooltips/`title` attributes were left showing the exact full number everywhere - only the visible text got shortened.

**Verified:** `node --check` clean; functional/sandbox tests confirming both the shortening math and that tooltips still carry exact values.

## Settings modal split into General / Agent Conf / Advanced Server Settings tabs

**What:** the Settings modal was one long unsegmented page (refresh intervals, SMS, notifications, Docker state change, Stats DB, then Advanced Server Settings tacked on at the bottom as a trailing full-width block). Requested: split it into tabs, move Advanced Server Settings into its own (last) tab, and add a new tab for editing a selected rig's `rigcontrol-agent.conf` remotely.

**How it works now:** `static/index.html`'s `#refresh-modal` body now opens with a `.settings-main-tabs` bar (General / Agent Conf / Advanced Server Settings, in that order) driving three `.settings-main-tab-panel` panels - General holds the original two-column body plus its existing footer unchanged, Advanced Server Settings holds the original WS-push/missed-refresh fields, and Agent Conf is new. `static/js/app.js` adds `initSettingsMainTabs()`/`switchSettingsMainTab()` (same wiring pattern as the existing `initWdconfigMainTabs()`/`switchWdconfigMainTab()` and `initColorSchemeTabs()`/`switchColorSchemeTab()` pairs); `openRefreshModal()` resets to the General tab every time it opens. Each tab ends in its own `.cmd-footer`-styled toolbar (status text on the left, action buttons on the right) rather than one shared footer for the whole modal, matching how the rest of the app already docks per-panel actions.

The Agent Conf tab reuses the exact read/write pattern already established for the Watchdog config editor (`autoLoadWdConfigForSelectedRig()`/`buildWdConfigCommand()`) rather than inventing a new one: "Reload from Worker" sends `cat /etc/rigcontrol/rigcontrol-agent.conf` through the existing generic `/command` MQTT channel (requires exactly one worker selected), the reply comes back through `handleCommandResponse()`'s existing dispatch (`pendingAgentConfFetchRig`, mirroring `pendingWdConfigFetchRig`), and "Send it !" builds a `sudo mkdir -p ... && sudo tee ... <<'EOF' ... EOF` heredoc (optionally followed by `sudo systemctl restart rigcontrol-agent.service` if "Restart agent after apply" is checked) and routes it through the same Confirm-checkbox-gated Send Cmd modal as Watchdog's "Send it !" does. A "Clear" button loads a blank, fully-commented example template (`AGENT_CONF_DEFAULT_TEMPLATE`) instead of emptying the box, for a rig that doesn't have a conf yet. `TEMPLATES_CONFIG.agentconf` (path/dir/restart command) is server-editable via `static/config/templates.json`, same as `TEMPLATES_CONFIG.watchdog` already was. `syncOpenModulesToSelection()` got a new Agent Conf branch (gated on the Settings modal being open AND the Agent Conf tab specifically being the active one) so switching the checkbox-selected rig while the tab is open auto-reloads it, matching the existing Watchdog-tab behavior.

**Verified:** `node --check` clean; div-open/close balance checked programmatically across the whole modified `#refresh-modal` block and across the whole file after every edit (caught and fixed one missing closing `</div>` from the Advanced tab's footer restructure before delivery); every new element `id` cross-checked between `index.html` and `app.js`'s `getElementById()` calls.

## Header 'x' close buttons: fixed alignment, added box styling, added to 6 dialogs that were missing one

**What:** three separate but related issues, found while adding the Agent Conf tab above and then auditing the rest of the app for consistency: (1) the Miner Configuration modal's header 'x' sat right next to its title instead of at the far right of the header, because `.cmd-header` is `justify-content: flex-start` and nothing pushed the button past the title. (2) The generic `.cmd-header-close` class (used by the Send Command and Logs modals' 'x' buttons) only got a visible box border on hover (`border: 1px solid transparent` by default) - inconsistent with the Send Command modal's 'x', which has its own themed always-visible box via a dedicated `#btn-close-cmd-modal` rule. (3) Six dialogs had no header close button at all - Remote Unlock, Manage Pools, Save to Wallets, Set Quick Actions, Themes, and Paste Theme JSON - even though five of the six already had a footer Cancel/Close button doing the same job.

**Fixed:** added `margin-left: auto` to `.fs-miner-config-close` (fixes #1, scoped to just that button so the shared `.cmd-header` layout isn't touched for anything else). Changed `.cmd-header-close`'s default border from `1px solid transparent` to `1px solid var(--border-modal)` so it reads as a clickable box even before hovering, and gave `.fs-miner-config-close` (previously `border: none`) the same treatment (fixes #2). Added a `.cmd-header-close`-styled 'x' button to all 6 dialogs from #3, each wired to call the exact same close function/logic its existing footer Cancel/Close button already used - no new close behavior invented, just a second way to trigger the same one.

**Verified:** `node --check` clean; CSS brace balance checked programmatically; confirmed every new button `id` exists in both `index.html` and has a matching `addEventListener` in `app.js`.

## Agent Conf added to DB Backups

**What:** the Backups module could already back up/restore/preview/delete Flightsheets, Overclocks, Saved Commands, Watchdog Profiles, Wallets, Status Log, Server Config, and Templates to/from DynamoDB, but had no way to include the per-rig `rigcontrol-agent.conf` content now viewable/editable through the new Agent Conf tab - a rig going offline for good meant losing whatever agent conf had been configured for it, with no local record anywhere.

**How it works now:** both dashboard servers (`rigcontrol_dashboard_server_windows`/`rigcontrol_dashboard_server_pi`) gained `_agentconf_cache_file_path()` (a new local JSON file, `rigcontrol_agentconf_cache.json`, next to `rigcontrol_config.json`) and a new `"agentconf"` `BACKUP_TARGETS` entry using the exact same whole-file-as-one-blob pattern already used for `server_config`/`templates` (`_JsonFileStub`/`_json_file_scan`/`_json_file_restore`/`_json_file_wipe` - no new backend abstraction needed). Unlike `server_config`/`templates`, this file isn't something the server boots its own behavior from, so it deliberately isn't added to the frontend's `BACKUP_SERVER_FILE_TARGET_IDS` set - no "this could break the running server" restore warning, and local delete stays allowed, consistent with Flightsheets/Wallets/etc. A new `POST /api/agent-conf/save` endpoint upserts one rig's `{content, saved_at}` into that file's per-rig dict. `static/js/app.js`'s Agent Conf tab calls it in two places: on a successful "Reload from Worker" (inside `handleCommandResponse()`'s existing `pendingAgentConfFetchRig` branch), and optimistically right before "Send it !" dispatches a write (since the Confirm-checked path hands off to the free-form Send Cmd modal, which has no structured success callback to hook a save onto). The Backups list itself needed no frontend changes - `renderBackupFilesList()` already renders whatever `/api/backups/files` returns, with no per-target-id hardcoding.

**Verified:** `python3 -m py_compile` clean on both dashboard servers; standalone round-trip test of the save/scan/restore logic (multiple rigs, an update-in-place, then a full backup-blob restore into a fresh file) confirming the restored file byte-matches the original; confirmed `BACKUP_SERVER_FILE_TARGET_IDS` correctly does not include `"agentconf"`.
